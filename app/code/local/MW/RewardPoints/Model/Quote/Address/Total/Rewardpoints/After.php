        ...
        public function collect(Mage_Sales_Model_Quote_Address $address)
        {
            ...
            else if((int)Mage::helper('rewardpoints')->getRedeemPointsOnTax($store_id) == MW_RewardPoints_Model_Redeemtax::BEFORE)
            {
                if($address->getSubtotal() > 0)
                {
                    ...
                    $totalDiscountAmount = $baseTotalDiscountAmount = $subtotalExlTax - $quote->getMwRewardpointDiscount() + $allShippingAmount + $taxAmount;
                    ...
                    {
                        $address->setMwRewardpoint($quote->getMwRewardpoint());
                        $address->setMwRewardpointDiscountShow(Mage::helper('core')->currency($quote->getMwRewardpointDiscount(), false, false));
                        $address->setMwRewardpointDiscount($baseTotalDiscountAmount);
                    }
                    ...
                    $address->setSubtotalInclTax(Mage::helper('core')->currency($subtotalExlTax - $quote->getMwRewardpointDiscount() + $taxAmount, false, false));
                    $address->setBaseSubtotalTotalInclTax($subtotalExlTax - $quote->getMwRewardpointDiscount());
                    $address->setGrandTotal($totalDiscountAmount);
                    $address->setBaseGrandTotal($baseTotalDiscountAmount);
                }
            }
        }
