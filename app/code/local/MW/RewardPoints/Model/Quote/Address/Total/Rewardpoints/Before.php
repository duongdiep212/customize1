<?php
     ...
     * Time: 12:32 PM
     */
    class MW_RewardPoints_Model_Quote_Address_Total_Rewardpoints_Before extends Mage_Sales_Model_Quote_Address_Total_Abstract
    {
        ...
        {
            $this->setCode('reward_points_before');
        }

        public function collect(Mage_Sales_Model_Quote_Address $address)
        {
            $store_id = Mage::app()->getStore()->getId();
            if((int)Mage::helper('rewardpoints')->getRedeemPointsOnTax($store_id) == MW_RewardPoints_Model_Redeemtax::BEFORE)
            {
                ...
                foreach ($items as $item)
                {
                    ...
                    if($item->getHasChildren() && $item->isChildrenCalculated())
                    {
                        foreach ($item->getChildren() as $child)
                        {
                            ...
                            /* if exist condition and not exit actions -> just apply rule only once */
                            if(isset($reward_point_array[3])){
                                break;
                            }
        ...
        public function fetch(Mage_Sales_Model_Quote_Address $address)
        {
            $store_id = Mage::app()->getStore()->getId();
            if(
                (int)Mage::helper('rewardpoints')->getRedeemPointsOnTax($store_id) == MW_RewardPoints_Model_Redeemtax::BEFORE &&
                in_array(Mage::getStoreConfig(Mage_Tax_Model_Config::CONFIG_XML_PATH_BASED_ON, $store_id), array('shipping', 'billing')) &&
                $address->getAddressType() == Mage::getStoreConfig(Mage_Tax_Model_Config::CONFIG_XML_PATH_BASED_ON, $store_id)
            )
            {
        ...
        public function processRule(Mage_Sales_Model_Quote_Item_Abstract $item, $programs, $qty, $price, $baseSubtotalWithDiscount)
        {
            ...
            foreach ($programs as $program)
            {
                ...
                /* if exist condition and not exit actions -> just apply rule only once */
                $ConditionsIsExist = unserialize($rule->getData('conditions_serialized'));
                $ActionsIsExits = unserialize($rule->getData('actions_serialized'));
                if(isset($ConditionsIsExist['conditions'])){
                    /** not to do anything */
                }
                if(!isset($ActionsIsExits['conditions'])){
                    $result[3] = '1';
                }
                /* done */
