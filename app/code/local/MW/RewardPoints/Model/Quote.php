        ...
        public function collectTotalAfter($argv)
        {
            if(Mage::helper('rewardpoints')->moduleEnabled())
            {
                ...
                if($controller_name == "multishipping" && ($action_name == "overview" || $action_name == "overviewPost"))
                {
                    ...
                    $shipping_address = $argv->getQuote()->getAllShippingAddresses();

                    $amount = Mage::getSingleton('core/session')->getMwRewardpointDiscountShowTotal();

                    $point = $argv->getQuote()->getMwRewardpoint();
                    if($amount != 0)
                    {
                    ...
                    foreach ($shipping_address as $address)
                    {
                        ...
                        $customer_id = $argv->getQuote()->getCustomerId();
                    ...
                    if($amount != 0)
                    {
                        $argv->getQuote()->setGrandTotal((float)$argv->getQuote()->getGrandTotal() - $total_amount_dec)->save();
                        $argv->getQuote()->setBaseGrandTotal((float)$argv->getQuote()->getBaseGrandTotal() - $total_amount_dec)->save();
                    }
                }
                else
                {
