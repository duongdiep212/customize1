<?php
    ...
    class MW_RewardPoints_Model_Report extends Mage_Core_Model_Abstract
    {
        ...
        public function prepareCollection($data)
        {
            ...
            switch($data['report_range'])
            {
                ...
                case MW_RewardPoints_Model_Admin_Const::REPORT_RAGE_LAST_WEEK:
                    ...
                    $rangeDate = $this->_buildArrayDate(MW_RewardPoints_Model_Admin_Const::REPORT_RAGE_LAST_WEEK, $startDay, $endDay);
                ...
                if($type == 'multiday')
                {
                    foreach($rangeDate as $year => $months)
                    {
                        foreach($months as $month => $days)
                        {
                            ...
                            foreach($collection_redeemd as $redeemd)
                            {
                                if($redeemd->getMonth() == $month)
                                {
                                    foreach($days as $day)
                                    {
                                        ...
                                        {
                                            $_data['report']['redeemed'][$year."-".$month."-".$day]  = array($year, $month, $day, (int)$redeemd->getTotalRedeemedSum());
                                        }
                                    }
                                }
                            }
                            foreach($collection_reward as $reward)
                            {
                                if($reward->getMonth() == $month)
                                {
                                    foreach($days as $day)
                                    {
                                        ...
                                        {
                                            $_data['report']['rewarded'][$year."-".$month."-".$day]  = array($year, $month, $day, (int)$reward->getTotalRewardedSum());
                                        }
                                    }
                                }
                            }
        ...
        protected function _buildCollection(&$collection, $data, $group = true)
        {
            switch($data['report_range'])
            {
                case MW_RewardPoints_Model_Admin_Const::REPORT_RAGE_LAST_24H:
                    ...
                    $where = 'CONVERT_TZ(main_table.transaction_time, \'+00:00\', \'+'.$this->_calOffsetHourGMT().':00\')';
        ...
        protected function _calOffsetHourGMT()
        {
            ...
            $value =  Mage::getSingleton('core/date')->calculateOffset(Mage::app()->getStore()->getConfig('general/locale/timezone'))/60/60;
            return str_pad(ceil($value), 2, "0", STR_PAD_LEFT);
